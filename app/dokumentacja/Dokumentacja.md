...
*   **Stany Procesu (`process_status`):**
    ...
    *   `FILTRACJA_PLACEK_KOLO`, `FILTRACJA_PRZELEW`, `FILTRACJA_KOLO`, `FILTRACJA_WYDMUCH`: Różne typy aktywnych cykli filtracyjnych.
    ...
    *   `W_MAGAZYNIE_CZYSTYM`: Mieszanina w magazynie czystym (beczka_czysta).
...
*   **Kluczowe Operacje i Przejścia Stanów:**
    ...
    2.  **Dobielanie:** Operator wykonuje akcję dodania ziemi (`add_bleaching_earth`).
        *   **Warunki:** Temperatura reaktora musi wynosić >= 110°C. Mieszaniny wydmuchowe (`is_wydmuch_mix=True`) mogą być dobielane.
...
*   Mieszaniny wydmuchowe mogą być dobielane (zmiana polityki względem wcześniejszej wersji).
