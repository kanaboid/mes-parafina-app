...
def create_app(config_class=None):
    ...
    def create_read_sensors_job(seconds=30):
        ...
        if existing_job:
            ...
            app.logger.info(f"🔄 Usunięto istniejące zadanie read_sensors przed utworzeniem nowego z interwałem {seconds}s")
        ...
        def read_sensors():
            ...
            with app.app_context():
                try:
                    ...
                    app.logger.error(f"Błąd podczas odczytu czujników: {str(e)}")
        
        # Zapisz referencję do zadania
        scheduler_jobs['read_sensors'] = read_sensors
        app.logger.info(f"✅ Utworzono nowe zadanie read_sensors z interwałem {seconds}s")
        return read_sensors
    ...
    def create_check_alarms_job(seconds=5):
        ...
        if existing_job:
            ...
            app.logger.info(f"🔄 [PID: {os.getpid()}] Usunięto istniejące zadanie check_alarms przed utworzeniem nowego z interwałem {seconds}s")
        ...
        app.logger.info(f"✅ [PID: {os.getpid()}] Utworzono nowe zadanie check_alarms z interwałem {seconds}s")
        return check_alarms
    ...
    def cleanup_existing_jobs():
        ...
        if existing_jobs:
            app.logger.info(f"🧹 Znaleziono {len(existing_jobs)} istniejących zadań - usuwam...")
            for job in existing_jobs:
                scheduler.remove_job(job.id)
                app.logger.info(f"🗑️ Usunięto zadanie: {job.id}")
        else:
            app.logger.info("✨ Brak istniejących zadań do usunięcia")
    ...
    # ZMIANA: Usunięcie całej logiki blokady plikowej i powrót do standardowej metody
    if not app.config.get('TESTING'):
        # Ten warunek jest kluczowy w trybie debugowania z reloaderem.
        ...
        if os.environ.get('WERKZEUG_RUN_MAIN') == 'true' or not app.config.get("DEBUG"):
            app.logger.info(f"🚀 [PID: {os.getpid()}] Główny proces roboczy. Inicjalizuję scheduler.")
            ...
            try:
                if not scheduler.running:
                    scheduler.start()
                    app.logger.info(f"✅ [PID: {os.getpid()}] Scheduler został uruchomiony.")
                else:
                    app.logger.info(f"ℹ️ [PID: {os.getpid()}] Scheduler już działa.")
            except Exception as e:
                app.logger.error(f"❌ [PID: {os.getpid()}] Błąd podczas uruchamiania schedulera: {e}")
        else:
            app.logger.info(f"🔍 [PID: {os.getpid()}] Proces-matka reloadera. Pomijam inicjalizację schedulera.")
